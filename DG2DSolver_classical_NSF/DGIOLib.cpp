...
namespace IO
{
    ...
    namespace loadSettingFiles
    {
        ...
        void massDiffSettings()
        {
            ...
            std::string fileName("FlowProperties.txt");
            std::string Loc(systemVar::wD + "/CASES/" + systemVar::caseName + "/System");
            std::string keyWordsDouble[numOfDouble] = {}, keyWordsInt[numOfInt] = {}, keyWordsBool[numOfBool] = {}, keyWordsStr[numOfStr] = {"totalE"};
            double outDB[numOfDouble] = {};
            int outInt[numOfInt] = {};
            bool outBool[numOfBool] = {};
            std::string outStr[numOfStr] = {};
            ...
            */
            ...
            if (outStr[0].compare("convective")==0)
            {
                flowProperties::mass_C=0;
            }
            else if (outStr[0].compare("total")==0)
            {
                flowProperties::mass_C=1;
            }
            else {
                std::string str0("Mode '"+outStr[0]+"' of FlowProperties>MassDiffusionSetting>totalE is not a available.");
                message::writeLog((systemVar::wD + "/CASES/" + systemVar::caseName), systemVar::caseName, str0);
            }
        }
    }
