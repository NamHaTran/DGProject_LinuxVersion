...
namespace auxUlti
{
    ...
    void prepareCaseForParallelRun()
    {
        //NOTE: phai chay ham nay SAU KHI da doc luoi vi ham nay can data cua luoi
        systemVar::parallelVars::totalNode=MPI::COMM_WORLD.Get_size();

        auxUlti::resize2DIntArray(systemVar::parallelVars::Elems2DDist, systemVar::parallelVars::totalNode, 2);
        auxUlti::resize2DIntArray(systemVar::parallelVars::Elems1DDist, systemVar::parallelVars::totalNode, 2);
        for (int inode=0;inode<systemVar::parallelVars::totalNode;inode++) {
            //Phan bo cell 2D vao cac processor
            systemVar::parallelVars::Elems2DDist[inode][0]=meshVar::nelem2D*inode/systemVar::parallelVars::totalNode;
            systemVar::parallelVars::Elems2DDist[inode][1]=meshVar::nelem2D*(inode+1)/systemVar::parallelVars::totalNode-1;
            for (int nelement=systemVar::parallelVars::Elems2DDist[inode][0];nelement<systemVar::parallelVars::Elems2DDist[inode][1];nelement++) {

            }
            //Phan bo cell 1D vao cac processor
            systemVar::parallelVars::Elems1DDist[inode][0]=meshVar::inpoedCount*inode/systemVar::parallelVars::totalNode;
            systemVar::parallelVars::Elems1DDist[inode][1]=meshVar::inpoedCount*(inode+1)/systemVar::parallelVars::totalNode-1;
        }
    }
}
