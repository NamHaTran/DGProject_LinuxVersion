...
std::vector<std::vector<double>> NSFEqBCsImplement(int element, int edge, int nG)
{
    ...
    {
        Fluxes = NSFEqBCs::wall::wallAdiabatic(element, edge, nG);
    }
    ...
    {
        Fluxes = NSFEqBCs::wall::wall_NonEquilibrium(element, edge, nG);
    }
    else
    {
...
namespace NSFEqBCs
{
    namespace wall
    {
        ...
        std::vector <std::vector<double>> wall_NonEquilibrium(int element, int edge, int nG)
        {
            ...
            double a(0.0), b(0.0), TPlus(0.0),TMinus(SurfaceBCFields::TBc[loc]), nx(auxUlti::getNormVectorComp(element, edge, 1)), ny(auxUlti::getNormVectorComp(element, edge, 2));
            std::tie(a, b) = auxUlti::getGaussSurfCoor(edge, element, nG);
            norm[0] = nx;
            norm[1] = ny;
    ...
    namespace patch
    {
        ...
        std::vector <std::vector<double>> outFlow(int element, int edge, int edgeGrp, int nG)
        {
            ...
            */
            ...
            if (BCSupportFncs::checkInflow(UPlus[1]/UPlus[0],UPlus[2]/UPlus[0], nx, ny)) //Tai outlet neu co inflow thi bi reversed flow
            {
