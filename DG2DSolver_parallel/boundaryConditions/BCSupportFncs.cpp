...
namespace BCSupportFncs
{
    ...
    void decompseU(std::vector<double> &priVars, const std::vector<double> &U, double rhoX, double rhoY, double iniT, bool calcT)
    {
        ...
        */
        ...
        if (calcT)
        {
            ...
            {
                T=math::CalcTFromConsvVar_massDiff(U[0],U[1],U[2],U[3],rhoX,rhoY,iniT);
            }
            else
            {
    ...
    namespace auxilaryBCs {
        ...
        void calcUMean(int element, std::vector<double> &UMean)
        {
            ...
            if (flowProperties::massDiffusion)
            {
                ...
                double rhoXP(BR1Vars::massDiffusion::rhoX[element][0]), rhoYP(BR1Vars::massDiffusion::rhoY[element][0]);

                TMean=math::CalcTFromConsvVar_massDiff(UMean[0],UMean[1],UMean[2],UMean[3],rhoXP,rhoYP,surfaceFields::T[edge][nG]);
                */
    ...
    {
        //Dung trong truong hop mass diffusion ON
        double rho(UMinus[0]),
                u(UMinus[1]/UMinus[0]),
                v(UMinus[2]/UMinus[0]);
        double mu(math::CalcVisCoef(T));

        double um(u-material::massDiffusion::DmCoeff*drhoX*mu),
                vm(v-material::massDiffusion::DmCoeff*drhoY*mu);

        return (rho*(material::Cv*T+0.5*(um*um+vm*vm)));
    }
    ...
    void NSFEqFluxes(int edgeId, std::vector<double> &Fluxes, double TPlus, double TMinus, std::vector<double> &UPlus, std::vector<double> &UMinus, std::vector<double> &dUXPlus, std::vector<double> &dUXMinus, std::vector<double> &dUYPlus, std::vector<double> &dUYMinus, std::vector<double> &normVector)
    {
        ...
        */
        process::NSFEq::calcLocalInviscidFlux(UPlus,invisFluxLocalXPlus,invisFluxLocalYPlus,ULPlus,normVector,dUXPlus[0],dUYPlus[0],TPlus);
        process::NSFEq::calcLocalInviscidFlux(UMinus,invisFluxLocalXMinus,invisFluxLocalYMinus,ULMinus,normVector,dUXMinus[0],dUYMinus[0],TMinus);
    ...
    void getUMinus_total(int edge, int nG, std::vector<double> &UMinus)
    {
        ...
        TM=surfaceFields::T[edge][mathVar::nGauss+nG+1];
