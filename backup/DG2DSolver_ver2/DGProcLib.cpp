...
namespace process
{
	...
	void calcVolumeGaussValues()
	{
        ...
        for (int nelem = 0; nelem < meshVar::nelem2D; nelem++)
		{
            for (int na = 0; na <= mathVar::nGauss; na++)
			{
				...
				{
					std::tie(a, b) = auxUlti::getGaussCoor(na, nb);
                    volumeFields::rhoVolGauss[nelem][na][nb] = math::pointValue(nelem, a, b, 1, 2);
                    volumeFields::rhouVolGauss[nelem][na][nb] = math::pointValue(nelem, a, b, 2, 2);
                    volumeFields::rhovVolGauss[nelem][na][nb] = math::pointValue(nelem, a, b, 3, 2);
                    volumeFields::rhoEVolGauss[nelem][na][nb] = math::pointValue(nelem, a, b, 4, 2);
                    volumeFields::T[nelem][na][nb] = math::CalcTFromConsvVar(volumeFields::rhoVolGauss[nelem][na][nb],volumeFields::rhouVolGauss[nelem][na][nb],volumeFields::rhovVolGauss[nelem][na][nb],volumeFields::rhoEVolGauss[nelem][na][nb]);
				}
			}
		}
	}
	...
	namespace auxEq
	{
		...
		std::vector<std::vector<double>> getGaussMatrixOfConserVar(int element, int valType)
		{
            ...
            double muGs(0.0), val(0.0);
            for (int na = 0; na <= mathVar::nGauss; na++)
			{
                for (int nb = 0; nb <= mathVar::nGauss; nb++)
				{
                    muGs=math::CalcVisCoef(volumeFields::T[element][na][nb]);
					switch (valType)
					{
					case 1:
					{
                        val = volumeFields::rhoVolGauss[element][na][nb];
					}
					break;
					case 2:
					{
                        val = volumeFields::rhouVolGauss[element][na][nb];
					}
					break;
					case 3:
					{
                        val = volumeFields::rhovVolGauss[element][na][nb];
					}
					break;
					case 4:
					{
                        val = volumeFields::rhoEVolGauss[element][na][nb];
					}
					break;
					default:
						break;
	...
	namespace timeDiscretization
	{
		...
		double localTimeStep(int element)
		{
            ...
            deltaT = fabs((1.0 / pow(mathVar::orderElem + 1, 2))*(size*systemVar::CFL) / (aSound + velocity + muVal));
			return deltaT;
		}
