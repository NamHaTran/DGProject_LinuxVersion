...
Bản backup 21/05/2021 - Tách sang nhánh Reese-Greenshield NSF eqns:
- Tách mu ra khỏi biến phụ mu*div(U) trước khi đưa vào hàm decompose div(U) để tính div của primary vars. Sau khi correct div(priVars) theo điều kiện biên rồi mới nhân lại với mu trước khi dùng để giải pt NSF. Việc remove mu này để enable khả năng apply điều kiện biên fixed Gradient, input giá trị gradient khác 0 trong tương lai.
- Enable mass diffusion tại wall.
- Tách bản code này thành 1 nhánh riêng:
	+ Nhánh này giải phương trình NSF mở rộng theo Reese-Greenshields, theo paper của Franz Durst thì mô hình này thiếu hiện tượng khuếch tán do gradient T.